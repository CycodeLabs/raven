...
def generate() -> None:
    ...
    for detection in detections:
        for result in detection.get("results"):
            table_data.append([detection.get("name"), detection.get("description"), result.get('w.path')])
    ...
    if Config.slack:
        if Config.slack_token and Config.channel_id:
            ...
            print("[x] Please provide slack token and channel id to send report to slack.")